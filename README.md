    ...
    `depsearch` scans a GhostBSD or FreeBSD ports tree and lists ports whose `Makefile` declares dependencies that match **your** patterns. Patterns are provided in a text file you control.
...
| sudo xargs -I{} sed -i '' -E \
  '/^(USES|LIB_DEPENDS|BUILD_DEPENDS|RUN_DEPENDS|USE_.*)=/I {
    ...
    - If you want full make evaluation later, we can add an optional mode that shells out to `make -V` in each port.
